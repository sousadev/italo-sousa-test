# Desafio técnico — Relatório consolidado de tráfego pago (versão enxuta)

> **Escopo desta etapa:** foque em **dois** dos quatro problemas abaixo — o
> **Módulo de dinheiro** e a **Ingestão idempotente** — mais o `DECISIONS.md`.
> As seções de **Relatório consolidado** e **Reconciliação de gaps** ficam
> documentadas como referência, mas **não precisam ser implementadas agora**
> (pode deixar os stubs como estão). Estimativa: ~3–4h.
...
**Essencial (o que avaliamos nesta etapa):**
- **módulo de dinheiro** (`MoneyService#calculate`) seguindo a ordem de
  operações acima, sem ponto flutuante binário no caminho do dinheiro;
...
- o **`DECISIONS.md`** — responda pelo menos as perguntas 1 a 4, 6 e 7.

Não precisa entregar tudo perfeito. Preferimos ver **suas decisões e seu
raciocínio** do que completude.

**Não precisa fazer agora (fica pra uma eventual 2ª etapa):**
- relatório consolidado (`GET /reports`);
- reconciliação de gaps (`GET /reconciliation/gaps`).

Pode deixar `getReport` e `findGaps` como `throw new Error('not implemented')`.
